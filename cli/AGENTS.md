...
**ALWAYS read this file FIRST when working on CLI tasks.** This prevents convention mixing between services.

### Why This Matters
- **Language**: This service uses **Rust**, not Python
- **Architecture**: This service follows **Elm Architecture (ratatui)**, not FastAPI patterns
- **Tooling**: Use `cargo clippy`, `cargo test`, **not** `ruff` or `pytest`
- **File Structure**: `src/main.rs`, `src/app.rs`, `src/ui.rs` - **not** `app/` layout

### Context Discipline
1. **Read this file first** - Loads Rust/ratatui conventions into context
2. **Scope all operations** - Use `path="cli/"` in Grep/Glob/Explore tools
3. **No cross-service pollution** - Don't read `api-service/` or `sync-service/` files unless explicitly needed
4. **Service switching** - When switching to another service, read its AGENTS.md first

**If asked to work on Python/FastAPI code, you're in the wrong service. Read `api-service/AGENTS.md` instead.**
